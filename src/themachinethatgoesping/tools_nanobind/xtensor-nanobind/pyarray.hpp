...
namespace nanobind
{
    namespace detail
    {
        ...
        template <class T, xt::layout_type L>
        struct type_caster<xt::xexpression<xt::pyarray<T, L>>> : type_caster<xt::pyarray<T, L>>
        {
            using Base = type_caster<xt::pyarray<T, L>>;
...
namespace xt
{
    ...
    template <class T, layout_type L>
    inline pyarray<T, L>::pyarray(const nb::object& o)
        : base_type(o)
    {
        init_from_python();
    }
    ...
    template <class T, layout_type L>
    inline auto pyarray<T, L>::ensure(nb::handle h) -> self_type
    {
        return base_type::ensure(h);
    }

    template <class T, layout_type L>
    inline bool pyarray<T, L>::check_(nb::handle h)
    {
