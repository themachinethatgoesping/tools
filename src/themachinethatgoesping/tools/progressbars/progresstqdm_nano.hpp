 ...
 * project is not linked against nanobind (not default for themachinethatgoesping_tools).
 *
 * This class is based in I_ProgressBarTimed such that the callbacks are guarded by a timer.
 */
class ProgressTqdmNano
    : public I_ProgressBarTimed
    , public nanobind::object
{
    double _first = 0;
  ...
  public:
     ...
     */
    ProgressTqdmNano(nanobind::object tqdm)
        : nanobind::object(tqdm)
    {
    }
    ~ProgressTqdmNano() { attr("close")(); }
  ...
  private:
    ...
    void callback_set_progress(double new_progress) override
    {
        ...
        double increment = new_progress - _first - nanobind::cast<double>(attr("n")());
    ...
    double callback_current() const override
    {
        ...
        return nanobind::cast<double>(attr("n")());
    }
};
