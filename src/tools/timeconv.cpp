...
unixtime_to_timepoint(double UnixTime)
{
...
double
timepoint_to_unixtime(chrono::system_clock::time_point TimePoint)
{
...
double
datestring_to_unixtime(const string& DateString, const string& format)
{
...
string
unixtime_to_datestring(double UnixTime,
    ...
    if (fractionalSecondsDigits == 0) {
      ...
      if (pos + fractionalSecondsDigits > datestring.size())
        fractionalSecondsDigits =
          static_cast<unsigned int>(datestring.size() - pos);

      datestring.replace(
        pos + fractionalSecondsDigits + 1, datestring.size(), "");
    }
  }
