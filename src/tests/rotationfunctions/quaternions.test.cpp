...
TEST_CASE("rotationfunctions: quaternion / rpy conversion", TESTTAG)
{
    SECTION("quaternion / rpy conversion should be reversible when within specified ranges")
    {
        // random generator. Use fixed seed to ensure repeatable results
        boost::random::mt19937 gen(123456);
        ...
        for (unsigned int i = 0; i < 1000; ++i)
        {
            ...
            std::array<double, 3> test = { roll, pitch, yaw };

            auto q1 = rotationfunctions::quaternion_from_rpy(roll, pitch, yaw);
            auto q2 = rotationfunctions::quaternion_from_rpy(test);

            REQUIRE(rotationfunctions::rpy_from_quaternion(q1) ==
                    rotationfunctions::rpy_from_quaternion(q2));
        }
    }

    SECTION("quaternion / rpy conversion should place yaw pitch and roll into defined limits")
    {
        std::vector<std::pair<std::array<double, 3>, std::array<double, 3>>> YPR = {
            { { -720., 0., -720. }, { 0., 0., 0. } }, // 0
            { { -540., -90., -540. },
        ...
        for (unsigned int c = 0; c < YPR.size(); ++c)
        {
            auto rpy = YPR[c].first;
            auto e   = YPR[c].second;
            std::swap(rpy[0], rpy[2]); // swap roll and yaw (to match the order of the function call
            //std::swap(e[0], e[2]); // swap roll and yaw (to match the order of the function call

            auto rpy_new =
                rotationfunctions::rpy_from_quaternion(rotationfunctions::quaternion_from_rpy(rpy));

            for (unsigned int i = 0; i < 3; ++i)
            {
                INFO("value: " << c << '/' << i << " / " << rpy[i] << " / " << rpy_new[i] << " / "
                               << e[i]);
                REQUIRE(rpy_new[i] == Catch::Approx(e[i]).scale(1));
            }
        }
    }
    ...
    SECTION("vectorized calls should produce the same results as single calls")
    {
        ...
        for (bool degrees : { true, false })
        {
            ...
            std::vector<std::array<double, 3>> RPY;
            std::vector<double>                Y, P, R;
            for (unsigned int i = 0; i < 1000; ++i)
            {
                auto yaw   = yaw_dist(gen);
                ...
                RPY.push_back({ roll, pitch, yaw });
                Y.push_back(yaw);
                P.push_back(pitch);
                R.push_back(roll);
            }

            auto Q1          = rotationfunctions::quaternion_from_rpy(RPY, degrees);
            auto Q2          = rotationfunctions::quaternion_from_rpy(R, P, Y, degrees);
            auto RPY_result1 = rotationfunctions::rpy_from_quaternion(Q1, degrees);
            auto RPY_result2 = rotationfunctions::rpy_from_quaternion(Q2, degrees);

            for (unsigned int c = 0; c < RPY.size(); ++c)
            {
                auto q   = rotationfunctions::quaternion_from_rpy(RPY[c], degrees);
                auto rpy = rotationfunctions::rpy_from_quaternion(q, degrees);
                ...
                {
                    INFO("degrees = " << degrees);
                    CHECK(RPY_result1[c][i] == Catch::Approx(rpy[i]));
                    CHECK(RPY_result2[c][i] == Catch::Approx(rpy[i]));
                }
            }
        }
    }
