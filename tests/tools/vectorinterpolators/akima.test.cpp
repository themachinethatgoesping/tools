          ...
          TESTTAG)
{
  ...
  REQUIRE_THROWS_AS(vectorinterpolators::AkimaInterpolator(
                      x_wrong_order, y_wrong_order, false, true),
                    std::runtime_error);

  // do not throw because vector is sorted this time
  vectorinterpolators::AkimaInterpolator interpolator_sorted(
  ...
  REQUIRE_THROWS_AS(
    vectorinterpolators::AkimaInterpolator(x_duplicates, y_duplicates, true, true),
    std::runtime_error);
}
