from themachinethatgoesping.tools import interpolation as ip

import pytest


#define class for grouping (test sections)
class Test_tools_interpolation_nearest:
  ...
  def test_NearestInterpolator_should_sort_and_check_lists(self):
    ...
    with pytest.raises(RuntimeError):
      ip.NearestInterpolator(X_wrong_order, Y_wrong_order)

    #check of exceptions are raise (duplicates should raise even if sortX is active)
    with pytest.raises(RuntimeError):
      ip.NearestInterpolator(X_duplicates, Y_duplicates, sortX=True)

    #sorting should cause X,Y _ wrong_order to be the same as XY
    data_xy_1 = ip.NearestInterpolator(X,Y).get_data_XY()
    data_xy_2 = ip.NearestInterpolator(X_wrong_order,Y_wrong_order,sortX=True).get_data_XY()
  ...
  def test_NearestInterpolator_should_perform_basic_interpolations(self):
    X = [-10, -5, 0, 6, 12]
    Y = [1, 0, 1, 0, -1]
    interpolator = ip.NearestInterpolator(X,Y)
    ...
    interpolator = ip.NearestInterpolator(X,Y, extrapolation_mode=ip.t_extr_mode.fail)
    ...
    interpolator = ip.NearestInterpolator(X,Y, extrapolation_mode=ip.t_extr_mode.nearest)
    ...
    interpolator = ip.NearestInterpolator(X,Y, extrapolation_mode=ip.t_extr_mode.extrapolate)
    assert interpolator.interpolate(-11) == pytest.approx(Y[0])
    assert interpolator.interpolate(13) == pytest.approx(Y[-1])
