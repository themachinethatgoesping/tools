...
class Test_tools_interpolation_nearest:
  ...
  def test_LinearInterpolator_should_sort_and_check_lists(self):
    ...
    with pytest.raises(RuntimeError):
      ip.LinearInterpolator(X_wrong_order, Y_wrong_order)

    #check of exceptions are raise (duplicates should raise even if sortX is active)
    with pytest.raises(RuntimeError):
      ip.LinearInterpolator(X_duplicates, Y_duplicates, sortX=True)

    #sorting should cause X,Y _ wrong_order to be the same as XY
    data_xy_1 = ip.LinearInterpolator(X,Y).get_data_XY()
    data_xy_2 = ip.LinearInterpolator(X_wrong_order,Y_wrong_order,sortX=True).get_data_XY()
  ...
  def test_LinearInterpolator_should_perform_basic_interpolations(self):
    X = [-10, -5, 0, 6, 12]
    Y = [1, 0, 1, 0, -1]
    interpolator = ip.LinearInterpolator(X,Y)
    ...
    interpolator = ip.LinearInterpolator(X,Y, extrapolation_mode=ip.t_extr_mode.fail)
    ...
    interpolator = ip.LinearInterpolator(X,Y, extrapolation_mode=ip.t_extr_mode.nearest)
    ...
    interpolator = ip.LinearInterpolator(X,Y, extrapolation_mode=ip.t_extr_mode.extrapolate)
